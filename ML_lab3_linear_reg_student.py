...
def parsePoint(line):
    ...
    """
    splited = line.split(',')
    return LabeledPoint(float(splited[0]), map(float, splited[1:]))
...
onlyLabels = parsedDataInit.map(lambda d: d.label)
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda d: LabeledPoint(d.label - minYear, d.features))
...
def calcRMSE(labelsAndPreds):
    ...
    return math.sqrt(labelsAndPreds.map(lambda lp: squaredError(lp[0], lp[1])).sum() / num)
...
labelsAndPreds = parsedValData.map(lambda d: (d.label, averageTrainYear))
...
def twoWayInteractions(lp):
    ...
    """
    label = lp.label
    features = list(lp.features) + map(lambda (x, y): x * y, itertools.product(lp.features, repeat=2))
