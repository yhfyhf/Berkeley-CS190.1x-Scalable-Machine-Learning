...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    indices = sorted(map(lambda r: OHEDict[r], rawFeats))
...
def parsePoint(point):
    ...
    """
    return [(idx, value) for (idx, value) in enumerate(point.split(',')[1:])]
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    indices = sorted(map(lambda r: OHEDict[r], filter(lambda r: OHEDict.get(r), rawFeats)))
    values = [1] * len(indices)
    return SparseVector(numOHEFeats, indices, values)
...
def computeLogLoss(p, y):
    ...
    """
    epsilon = 10e-12
    p = epsilon if p - 0.0 < epsilon else p
    p = 1 - epsilon if 1 - p < epsilon else p
...
classOneFracTrain = OHETrainData.map(lambda lp: lp.label).reduce(lambda x, y: x + y) / nTrain
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    sparcedFeatures = SparseVector(numBuckets, hashFunction(numBuckets, parsePoint(point), True))
    return LabeledPoint(point.split(',')[0], sparcedFeatures)
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda x: len(x.features.indices) / float(d)).reduce(lambda x, y: x + y) * 1. / n
